...
typedef struct {
  DeviceIO_Status_t (*read_registers)(void *context, uint8_t device_address,
                                      uint8_t start_register, uint8_t *data,
                                      uint16_t length, uint32_t timeout_ms);
  DeviceIO_Status_t (*write_registers)(void *context, uint8_t device_address,
                                       uint8_t start_register,
                                       const uint8_t *data, uint16_t length,
                                       uint32_t timeout_ms);
  DeviceIO_Status_t (*read_registers_it)(void *context, uint8_t device_address,
                                         uint8_t start_register, uint8_t *data,
                                         uint16_t length);
