...
void MotorPWM_NotiStatus(MotorPWM_Status_t status, const char *str) {
  if (str != NULL && str[0] != '\0') {
    printf("%s\r\n", str);
  }
  ...
  case MOTOR_PWM_ERR_FAULT:
    printf("MOTOR_PWM_ERR_FAULT\r\n");
    break;
...
MotorPWM_Status_t MotorPWM_Start(MotorPWM_Handle_t *motor_pwm,
                                 uint16_t initial_pulse_us) {
  if (motor_pwm == NULL || motor_pwm->htim == NULL) {
    return MOTOR_PWM_ERR_NULL;
  }

  if (motor_pwm->state == MOTOR_PWM_STATE_UNINITIALIZED) {
    return MOTOR_PWM_ERR_NOT_INITIALIZED;
  }

  if (motor_pwm->state == MOTOR_PWM_STATE_STARTED) {
    return MOTOR_PWM_ERR_ALREADY_STARTED;
  }

  if (motor_pwm->state == MOTOR_PWM_STATE_FAULT) {
    return MOTOR_PWM_ERR_FAULT;
  }
  ...
  const uint32_t compare_ticks = initial_pulse_us;

  for (uint32_t i = 0U; i < MOTOR_PWM_QUANTITY; ++i) {
    __HAL_TIM_SET_COMPARE(motor_pwm->htim, motor_pwm->config.channels[i],
                          compare_ticks);
  }

  __HAL_TIM_SET_COUNTER(motor_pwm->htim, 0U);

  if (HAL_TIM_GenerateEvent(motor_pwm->htim, TIM_EVENTSOURCE_UPDATE) !=
      HAL_OK) {
    motor_pwm->state = MOTOR_PWM_STATE_STOPPED;
    return MOTOR_PWM_ERR_HAL;
  }

  __HAL_TIM_CLEAR_FLAG(motor_pwm->htim, TIM_FLAG_UPDATE);

  uint32_t started_channels = 0U;

  for (uint32_t i = 0U; i < MOTOR_PWM_QUANTITY; ++i) {
    if (HAL_TIM_PWM_Start(motor_pwm->htim, motor_pwm->config.channels[i]) !=
        HAL_OK) {
      uint8_t rollback_ok = 1U;

      for (uint32_t j = 0U; j < started_channels; ++j) {
        if (HAL_TIM_PWM_Stop(motor_pwm->htim, motor_pwm->config.channels[j]) !=
            HAL_OK) {
          rollback_ok = 0U;
        }
      }
      ...
      motor_pwm->state =
          (rollback_ok != 0U) ? MOTOR_PWM_STATE_STOPPED : MOTOR_PWM_STATE_FAULT;
...
MotorPWM_Status_t
MotorPWM_WriteAllUs(MotorPWM_Handle_t *motor_pwm,
                    const uint16_t pulse_us[MOTOR_PWM_QUANTITY]) {
  if (motor_pwm == NULL || motor_pwm->htim == NULL || pulse_us == NULL) {
    return MOTOR_PWM_ERR_NULL;
  }
  ...
  if (motor_pwm->state == MOTOR_PWM_STATE_STOPPED) {
...
MotorPWM_Status_t MotorPWM_WriteAllSameUs(MotorPWM_Handle_t *motor_pwm,
                                          uint16_t pulse_us) {
  if (motor_pwm == NULL || motor_pwm->htim == NULL) {
    return MOTOR_PWM_ERR_NULL;
  }
...
MotorPWM_Status_t MotorPWM_Stop(MotorPWM_Handle_t *motor_pwm) {
  if (motor_pwm == NULL || motor_pwm->htim == NULL) {
    return MOTOR_PWM_ERR_NULL;
  }
  ...
  for (uint32_t i = 0U; i < MOTOR_PWM_QUANTITY; ++i) {
    if (HAL_TIM_PWM_Stop(motor_pwm->htim, motor_pwm->config.channels[i]) !=
        HAL_OK) {
...
MotorPWM_Status_t MotorPWM_Init(MotorPWM_Handle_t *motor_pwm,
                                TIM_HandleTypeDef *htim,
                                const MotorPWM_Config_t *config) {
  if (motor_pwm == NULL || htim == NULL || config == NULL) {
    return MOTOR_PWM_ERR_NULL;
  }
  ...
  if (htim->Instance == NULL || config->frame_period_us == 0U) {
