...
static uint8_t esc_is_initialized(const ESC_Handle_t *esc) {
  return esc != NULL && esc->initialized != 0U && esc->motor_pwm != NULL;
}

static uint8_t esc_is_valid_throttle(float throttle) {
  return throttle >= 0.0f && throttle <= 1.0f;
}

static uint16_t esc_throttle_to_pulse_us(const ESC_Handle_t *esc,
                                         float throttle) {
  const float pulse =
      (float)esc->config.idle_pulse_us +
      throttle * (float)(esc->config.max_pulse_us - esc->config.idle_pulse_us);
...
ESC_Status_t ESC_Init(ESC_Handle_t *esc, MotorPWM_Handle_t *motor_pwm,
  ...
  if (esc == NULL || motor_pwm == NULL || config == NULL) {
    return ESC_ERR_NULL;
  }
...
ESC_Status_t ESC_Start(ESC_Handle_t *esc) {
  ...
  const MotorPWM_Status_t status =
      MotorPWM_Start(esc->motor_pwm, esc->config.stop_pulse_us);
...
ESC_Status_t ESC_WriteStop(ESC_Handle_t *esc) {
  ...
  const MotorPWM_Status_t status =
      MotorPWM_WriteAllSameUs(esc->motor_pwm, esc->config.stop_pulse_us);
...
ESC_Status_t ESC_SetThrottleAll(ESC_Handle_t *esc,
  ...
  for (uint32_t i = 0U; i < MOTOR_PWM_QUANTITY; ++i) {
    ...
    pulses[i] = esc_throttle_to_pulse_us(esc, throttles[i]);
  }

  const MotorPWM_Status_t status = MotorPWM_WriteAllUs(esc->motor_pwm, pulses);
...
ESC_Status_t ESC_SetThrottleAllSame(ESC_Handle_t *esc, float throttle) {
  ...
  const uint16_t pulse_us = esc_throttle_to_pulse_us(esc, throttle);

  const MotorPWM_Status_t status =
      MotorPWM_WriteAllSameUs(esc->motor_pwm, pulse_us);
...
ESC_Status_t ESC_Stop(ESC_Handle_t *esc) {
  ...
  const MotorPWM_Status_t status = MotorPWM_Stop(esc->motor_pwm);
