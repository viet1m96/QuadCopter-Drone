...
static MPU6050_Status_t mpu6050_convert_io_status(
        DeviceIO_Status_t io_status)
{
    switch (io_status) {
    case DEVICE_IO_OK:
        return MPU6050_OK;
...
static uint8_t mpu6050_io_is_valid(
        const DeviceIO_t *io)
{
    if (io == NULL
            || io->context == NULL
    ...
    if (io->ops->write_registers == NULL
            || io->ops->read_registers == NULL
            || io->ops->read_registers_it == NULL
            || io->ops->delay_ms == NULL
...
static MPU6050_Status_t mpu6050_check_initialized(
        ...
        const MPU6050_Config_t *config)
{
    ...
    if (config->clksrc == 6U
            || config->clksrc > MPU6050_CLKSRC_STOPCLK) {
        return MPU6050_INVALID_CONFIG;
    }
    ...
    if (config->fs_sel_config
            > MPU6050_GYRO_CONFIG_FS_2000DPS) {
        return MPU6050_INVALID_CONFIG;
    }

    if (config->accel_sel_config
            > MPU6050_ACCEL_CONFIG_AFS_16G) {
        return MPU6050_INVALID_CONFIG;
    }
...
static MPU6050_Status_t mpu6050_write_register(
        MPU6050_Handle_t *mpu,
        uint8_t register_address,
        uint8_t value)
{
    DeviceIO_Status_t io_status =
            mpu->io->ops->write_registers(
                    mpu->io->context,
                    mpu->config.address,
                    register_address,
                    &value,
    ...
    return mpu6050_convert_io_status(
            io_status);
}

static MPU6050_Status_t mpu6050_read_registers(
        MPU6050_Handle_t *mpu,
...
static MPU6050_Status_t mpu6050_read_register(
        MPU6050_Handle_t *mpu,
        uint8_t register_address,
        uint8_t *value)
{
...
static void mpu6050_parse_vector(
        const uint8_t *buffer,
        Vector3i16_t *vector)
{
    vector->x =
            byte_utils_i16_from_be(
                    buffer[0],
                    buffer[1]);
...
static void mpu6050_parse_raw_buffer(
        const uint8_t *buffer,
        MPU6050_RawData_t *raw)
{
    mpu6050_parse_vector(
            &buffer[0],
            &raw->accel);
...
static MPU6050_Status_t mpu6050_set_gyro_range(
        MPU6050_Handle_t *mpu,
        uint8_t fs_sel)
{
    MPU6050_Status_t status =
            mpu6050_update_bits(
                    ...
                    (uint8_t)(
                            MPU6050_CONFIG_FS_SEL_MASK
                            << MPU6050_CONFIG_FS_SEL_POS),
                    (uint8_t)(
                            fs_sel
                            << MPU6050_CONFIG_FS_SEL_POS));
    ...
    case MPU6050_GYRO_CONFIG_FS_500DPS:
        mpu->gyro_scale =
                MPU6050_GYRO_SENS_500DPS;
        break;

    case MPU6050_GYRO_CONFIG_FS_1000DPS:
        mpu->gyro_scale =
                MPU6050_GYRO_SENS_1000DPS;
        break;

    case MPU6050_GYRO_CONFIG_FS_2000DPS:
        mpu->gyro_scale =
                MPU6050_GYRO_SENS_2000DPS;
        break;
...
static MPU6050_Status_t mpu6050_set_accel_range(
        MPU6050_Handle_t *mpu,
        uint8_t afs_sel)
{
    MPU6050_Status_t status =
            mpu6050_update_bits(
                    ...
                    (uint8_t)(
                            MPU6050_CONFIG_AFS_SEL_MASK
                            << MPU6050_CONFIG_AFS_SEL_POS),
                    (uint8_t)(
                            afs_sel
                            << MPU6050_CONFIG_AFS_SEL_POS));
...
static MPU6050_Status_t mpu6050_collect_still_samples(
        MPU6050_Handle_t *mpu,
        MPU6050_RawData_t *raw,
        ...
        MPU6050_CalibrationAverage_t *average)
{
    ...
    Vector3i16_t gyro_min = {
        .x = INT16_MAX,
        .y = INT16_MAX,
        .z = INT16_MAX
    };

    Vector3i16_t gyro_max = {
        .x = INT16_MIN,
        .y = INT16_MIN,
        .z = INT16_MIN
    };

    Vector3i16_t accel_min = {
        .x = INT16_MAX,
        .y = INT16_MAX,
        .z = INT16_MAX
    };

    Vector3i16_t accel_max = {
        .x = INT16_MIN,
        .y = INT16_MIN,
        .z = INT16_MIN
    };
    ...
    if ((float)(gyro_max.x - gyro_min.x)
                    > config->gyro_threshold
            || (float)(gyro_max.y - gyro_min.y)
                    > config->gyro_threshold
            || (float)(gyro_max.z - gyro_min.z)
                    > config->gyro_threshold) {
        return MPU6050_ERR_MOVING;
    }

    if ((float)(accel_max.x - accel_min.x)
                    > config->accel_axis_threshold
            || (float)(accel_max.y - accel_min.y)
                    > config->accel_axis_threshold
            || (float)(accel_max.z - accel_min.z)
                    > config->accel_axis_threshold) {
        return MPU6050_ERR_MOVING;
    }

    float count =
            (float)config->sample_count;
    ...
    float accel_magnitude =
            sqrtf(
                    average->accel_x * average->accel_x
        ...
        const MPU6050_Config_t *config)
{
    ...
    MPU6050_Status_t status =
            mpu6050_validate_config(config);
        ...
        MPU6050_Data_t *data)
{
    ...
    return MPU6050_ConvertRawToPhysical(
            mpu,
            &raw,
            data);
}
        ...
        MPU6050_Data_t *calibrated)
{
    ...
    calibrated->gyro_dps.x =
            physical->gyro_dps.x
            - mpu->gyro_offset_dps.x;

    calibrated->gyro_dps.y =
            physical->gyro_dps.y
            - mpu->gyro_offset_dps.y;

    calibrated->gyro_dps.z =
            physical->gyro_dps.z
            - mpu->gyro_offset_dps.z;
        ...
        MPU6050_StillnessConfig_t *stillness)
{
    ...
    stillness->sample_count =
            MPU6050_CALIB_SAMPLE_COUNT;

    stillness->sample_delay_ms =
            MPU6050_CALIB_SAMPLE_DELAY_MS;

    stillness->timeout_ms =
            MPU6050_CALIB_TIMEOUT_MS;

    stillness->gyro_threshold =
            MPU6050_GYRO_STILLNESS_DPS
            * mpu->gyro_scale;

    stillness->accel_axis_threshold =
            MPU6050_ACCEL_AXIS_STILLNESS_G
            * mpu->accel_scale;

    stillness->accel_threshold =
            MPU6050_ACCEL_MAGNITUDE_G
            * mpu->accel_scale;

    stillness->accel_allowed_gap =
            MPU6050_ACCEL_MAGNITUDE_GAP
            * mpu->accel_scale;
        ...
        const MPU6050_StillnessConfig_t *stillness)
{
    ...
    uint32_t start_time =
            mpu->io->ops->get_tick_ms(
                    mpu->io->context);

    for (;;) {

        MPU6050_Status_t status =
                mpu6050_collect_still_samples(
                        mpu,
                        raw,
                        stillness,
                        &average);
        ...
        const MPU6050_InterruptConfig_t *config)
{
    ...
    uint8_t mask =
            (uint8_t)(
                    (1U << MPU6050_INT_LEVEL_BIT)
                  | (1U << MPU6050_INT_OPEN_BIT)
    ...
    uint8_t value =
            (uint8_t)(
                    (config->int_level
                            << MPU6050_INT_LEVEL_BIT)
                  | (config->int_open
                            << MPU6050_INT_OPEN_BIT)
                  | (config->latch_int_en
                            << MPU6050_INT_LATCH_BIT)
                  | (config->int_rd_clear
                            << MPU6050_INT_RD_CLEAR_BIT));

    return mpu6050_update_bits(
            mpu,
