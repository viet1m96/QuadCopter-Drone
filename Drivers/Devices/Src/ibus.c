...
static uint16_t ibus_calculate_checksum(
...
static uint16_t ibus_get_received_checksum(
        const uint8_t *raw_frame)
{
...
IBUS_Status_t IBUS_DecodeFrame(
		const uint8_t *raw_frame,
		size_t frame_size,
		IBUS_Data_t *decoded_frame) {
	if(raw_frame == NULL || decoded_frame == NULL) return IBUS_ERR_NULL;
	if(frame_size != IBUS_FRAME_SIZE) return IBUS_ERR_FRAME_SIZE;
	...
	for(size_t i = 0U; i < IBUS_CHANNEL_COUNT; i++) {
		...
		decoded_frame->channels[i] = byte_utils_u16_from_be(raw_frame[idx + 1], raw_frame[idx]);
	}
