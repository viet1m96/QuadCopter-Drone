...
static DeviceIO_Status_t hal_i2c_convert_status(HAL_StatusTypeDef status) {
  switch (status) {
  case HAL_OK:
    return DEVICE_IO_OK;
...
static DeviceIO_Status_t
hal_i2c_write_registers(void *context, uint8_t device_address,
                        uint8_t start_register, const uint8_t *data,
  ...
  HAL_StatusTypeDef status = HAL_I2C_Mem_Write(
      hi2c, (uint16_t)(device_address << 1U), start_register,
      I2C_MEMADD_SIZE_8BIT, (uint8_t *)data, length, timeout_ms);

  return hal_i2c_convert_status(status);
}

static DeviceIO_Status_t hal_i2c_read_registers(void *context,
  ...
  HAL_StatusTypeDef status =
      HAL_I2C_Mem_Read(hi2c, (uint16_t)(device_address << 1U), start_register,
                       I2C_MEMADD_SIZE_8BIT, data, length, timeout_ms);

  return hal_i2c_convert_status(status);
}

static DeviceIO_Status_t hal_i2c_read_registers_it(void *context,
                                                   uint8_t device_address,
                                                   uint8_t start_register,
                                                   uint8_t *data,
                                                   uint16_t length) {
  ...
  HAL_StatusTypeDef status =
      HAL_I2C_Mem_Read_IT(hi2c, (uint16_t)(device_address << 1U),
                          start_register, I2C_MEMADD_SIZE_8BIT, data, length);

  return hal_i2c_convert_status(status);
}

static void hal_i2c_delay_ms(void *context, uint32_t delay_ms) {
  (void)context;

  HAL_Delay(delay_ms);
}

static uint32_t hal_i2c_get_tick_ms(void *context) {
  (void)context;
