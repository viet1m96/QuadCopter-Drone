...
static void NotifyReceiverTaskFromISR(
        uint32_t events,
        BaseType_t *higher_priority_task_woken)
{
    if (receiver_ctx == NULL
            || receiver_ctx->task_handle == NULL) {
        return;
    }
...
static void NotifySensorTaskFromISR(
        uint32_t events,
        BaseType_t *higher_priority_task_woken)
{
    if (sensor_ctx == NULL
        ...
        uint16_t gpio_pin)
{
    ...
    NotifySensorTaskFromISR(
            SENSOR_EVENT_MPU6050_DRDY,
            &higher_priority_task_woken);
