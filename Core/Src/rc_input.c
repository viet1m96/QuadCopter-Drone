...
rc_input_validate_throttle(const RCInput_ThrottleConfig_t *config) {
  if (config->channel_idx >= IBUS_CHANNEL_COUNT)
    return 0U;
...
static uint8_t rc_input_validate_dup_channel(const RCInput_Config_t *config) {
  return (config->throttle.channel_idx != config->roll.channel_idx &&
...
RCInput_Status_t RCInput_Init(RCInput_Handle_t *rc,
                              const RCInput_Config_t *config) {
  if (rc == NULL || config == NULL)
...
static float rc_input_clamp(float value, float min, float max) {
  if (value < min)
    return min;
  if (value > max)
    return max;
...
static float
rc_input_normalize_throttle(const RCInput_ThrottleConfig_t *throttle,
                            uint16_t raw) {
  float numerator = (float)((int32_t)raw - (int32_t)throttle->min);

  float denominator = (float)((int32_t)throttle->max - (int32_t)throttle->min);
...
static float rc_input_normalize_axis(const RCInput_AxisConfig_t *axis,
                                     uint16_t raw) {
  ...
  if ((raw_value >= lower_deadband) && (raw_value <= upper_deadband)) {
    normalized = 0.0f;
  } else if (raw_value < lower_deadband) {
    normalized = (float)(raw_value - lower_deadband) /
                 (float)(lower_deadband - (int32_t)axis->min);
  } else {
    normalized = (float)(raw_value - upper_deadband) /
...
static RCInput_Mode_t rc_input_detect_mode(uint16_t raw) {
  if (raw == FLIGHT_MODE_RATE)
    return RC_MODE_RATE;
  if (raw == FLIGHT_MODE_ANGLE)
    return RC_MODE_ANGLE;
  return 0;
}

static uint8_t rc_input_detect_arm_request(uint16_t raw) {
  return (raw == ARM_REQUESTED) ? 1U : 0U;
}

RCInput_Status_t RCInput_Convert(const RCInput_Handle_t *rc,
  ...
  command->roll = rc_input_normalize_axis(
      &rc->config.roll, data->channels[rc->config.roll.channel_idx]);

  command->pitch = rc_input_normalize_axis(
      &rc->config.pitch, data->channels[rc->config.pitch.channel_idx]);
  command->yaw = rc_input_normalize_axis(
      &rc->config.yaw, data->channels[rc->config.yaw.channel_idx]);

  command->failsafe_active =
      rc_input_detect_failsafe(data->channels[FAILSAFE_CHANNEL_IDX]);
  command->mode = rc_input_detect_mode(data->channels[FLIGHT_MODE_CHANNEL_IDX]);
  command->arm_request =
      rc_input_detect_arm_request(data->channels[ARM_REQUEST_CHANNEL_IDX]);
  return RC_INPUT_OK;
}
