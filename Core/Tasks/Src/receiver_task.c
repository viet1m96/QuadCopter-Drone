...
static void start_failsafe_protocol(ReceiverTask_Context_t *context,
...
static uint8_t receiver_start_next_frame(ReceiverTask_Context_t *context) {
  if (context == NULL || context->transport == NULL) {
    return 0U;
  }

  return HAL_IBUS_TransportStart(context->transport) == HAL_IBUS_TRANSPORT_OK;
}
...
static void ReceiverTask(void *argument) {
  ...
  for (;;) {
    ...
    TickType_t wait_ticks = context->timeout_ticks;
