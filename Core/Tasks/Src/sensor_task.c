...
static uint8_t DeadlineIsEarlier(
        TickType_t first,
        TickType_t second)
{
...
static SensorOwner_t SelectEarliestRequest(
        SensorRequestCandidate_t *candidates,
        uint32_t count)
{
    SensorOwner_t selected_owner =
            SENSOR_OWNER_NONE;
...
static void RecoverI2C1(
        SensorTask_Context_t *context)
{
    (void)context;
}

static void HandleMPU6050Data(
        SensorTask_Context_t *context)
{
    ...
    if (MPU6050_GetRawDataIT(
            context->imu,
            &raw) != MPU6050_OK) {
        return;
    }

    if (MPU6050_ConvertRawToPhysical(
            context->imu,
            &raw,
            &physical) != MPU6050_OK) {
        return;
    ...
    if (MPU6050_ApplyCalibration(
            context->imu,
            &physical,
            &calibrated) != MPU6050_OK) {
        return;
    }
    ...
    (void)xQueueOverwrite(
            context->data_queue_to_control,
            &calibrated);
}

static void HandleI2C1Events(
        SensorTask_Context_t *context,
        uint32_t events)
{
    SensorBusManager_t *bus =
            &context->i2c1_manager;
    ...
    if ((events & SENSOR_EVENT_MPU6050_DRDY) != 0U) {
        if (context->imu_request.pending == 0U) {
            ...
            context->imu_request.deadline_tick =
                    now
                    + context->imu_request.max_latency_ticks;
        }
    }
}
        ...
        TickType_t now)
{
    ...
    if (bus->state == SENSOR_BUS_BUSY) {
        ...
        DeviceIO_Status_t status =
                bus->device_io->ops->abort_it(
                        bus->device_io->context,
                        bus->active_device_address);
        ...
        TickType_t now)
{
    ...
    SensorRequestCandidate_t candidates[] = {
        {
            .owner = SENSOR_OWNER_MPU6050,
            .request = &context->imu_request
        }
    };

    SensorOwner_t owner =
            SelectEarliestRequest(
                    candidates,
                    sizeof(candidates)
                    / sizeof(candidates[0]));
    ...
    switch (owner) {
    case SENSOR_OWNER_MPU6050:
        if (MPU6050_StartReadRawDataIT(
                context->imu) != MPU6050_OK) {
            return;
        }
...
static TickType_t CalculateBusWaitTime(
        SensorBusManager_t *bus,
        TickType_t now)
{
    TickType_t timeout;
    ...
    if (bus->state == SENSOR_BUS_BUSY) {
        timeout =
                bus->transaction_timeout_ticks;
    } else if (bus->state == SENSOR_BUS_ABORTING) {
        timeout =
                bus->abort_timeout_ticks;
    } else {
        return portMAX_DELAY;
    }

    TickType_t elapsed =
            now - bus->state_started_tick;
...
static TickType_t CalculateWaitTime(
        SensorTask_Context_t *context,
        TickType_t now)
{
        ...
        SensorTask_Context_t *sensor_ctx)
{
    if (sensor_ctx == NULL
            || sensor_ctx->imu == NULL
            || sensor_ctx->imu->io == NULL
            || sensor_ctx->imu->io->ops == NULL
    ...
    sensor_ctx->imu_request.max_latency_ticks = pdMS_TO_TICKS(MPU6050_REQUEST_MAX_LATENCY_MS);
    ...
    sensor_ctx->i2c1_manager.transaction_timeout_ticks =pdMS_TO_TICKS(I2C1_TRANSACTION_TIMEOUT_MS);

    sensor_ctx->i2c1_manager.abort_timeout_ticks =pdMS_TO_TICKS(I2C1_ABORT_TIMEOUT_MS);
